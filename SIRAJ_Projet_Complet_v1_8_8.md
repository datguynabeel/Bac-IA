...
- v1.8.7 (2026-06-05) — A1 acté (§14.4.6 dérogation runtime free-tier + §14.6.6 produit fonctionnel non scripté) ; routage modèle rendu agnostique (§14.2.4, §14.2.3, §14.2.6) ; mapping modèles concret déporté dans lessons.md.
- v1.8.8 (2026-06-10) — Correction CNDP→MEN (curriculum) ; dégraissage méta-gouvernance (retrait checklist/compteur/proxies §14.2.6.5 + §14.2.9.1-2 ; jauge = % UI fondateur) ; règle anti-sycophantie §14.2.9.h ; dérogation test Sonnet borné §14.4.6 ; repivot terrain §14.6.8 ; note choix LLM §3.3 ; corrections mécaniques.
---
...
|---|---|---|---|
| 1 | **Spécialisation programme officiel marocain** | 3.4 (RAG sur le programme officiel MEN — Orientations Pédagogiques + Cadres de référence des examens nationaux + annales Bac National) | Ne connaît ni le découpage du programme officiel marocain (MEN), ni les coefficients par filière, ni le format exact des épreuves marocaines |
...
1. *"ChatGPT ne connaît pas le programme officiel marocain (MEN)."* (moat technique RAG)
...
- L'écart de moats 1, 2 et 5 dépend directement de la qualité d'ingestion RAG (Section 3.4) : tant que le pipeline d'ingestion du programme officiel marocain (MEN) n'est pas opérationnel à 100%, l'argument est partiellement déclaratif. **Priorité absolue Phase 1 : sécuriser le RAG marocain comme actif différenciant.**
...
```
1. Élève atteint le plafond de 5 questions/jour gratuit (freemium "Découverte enrichi V2", acté v1.4)
...
Note (2026-06-10) : runtime confirmed Sonnet 4.6 (suivi socratique, darija, marge). Seul challenger sérieux pour ce cas = Gemini 3.1 Pro (2/12 $, maths solides, multilingue, déjà dans le duo Anthropic+Google). GPT-5.x = meilleur en maths brutes mais 3e fournisseur (viole §14.2.4). DeepSeek/Qwen écartés du runtime (hébergement Chine vs données mineurs). Décideur final = test empirique darija + maths curriculum Sonnet vs Gemini, pendant le build du chapitre. Comparatif détaillé = veille Phase 2 (choix réversible 1 ligne via abstraction A1).
...
# PARTIE 13 — IDENTITÉ DE MARQUE & DESIGN
...
Antigravity est utilisé pour toute la production effective — code, scripts, refactoring, intégration. Le forfait Google AI Pro donne accès aux modèles disponibles avec quotas selon le plan Google AI en vigueur (fenêtre glissante ~5h depuis Antigravity 2.0, mai 2026), à Claude (modèles Anthropic) via Vertex Model Garden, à NotebookLM Pro (5× sources), à Deep Research illimité, et à Veo 3.1 pour la génération vidéo marketing.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 POUR ANTIGRAVITY
Modèle recommandé : [modèle courant — cf. tasks/lessons.md, mapping daté]
...
|---|---|
| Génération de code React depuis Stitch (skill `react:components`) | **Claude raisonnement (cf. lessons.md)** — non négociable, le skill est calibré Anthropic |
| Édition / refactoring de composants SIRAJ existants | **Claude raisonnement (cf. lessons.md)** — précision logique, moins d'over-engineering |
| Audit ou recherche cross-fichiers sur l'ensemble du repo (≥ 50 fichiers) | **Modèle long-contexte (cf. lessons.md)** — contexte 2M tokens |
| Script utilitaire, parsing PDF de manuel, format conversion, migration SQL | **Modèle rapide économique (cf. lessons.md)** — rapide, suffisant |
| Système prompt élève en darija/arabe à tester | **Claude raisonnement (cf. lessons.md)** — meilleur ancrage culturel multilingue |
| Génération de QCM ou exercices de math depuis programme officiel | **Claude raisonnement (cf. lessons.md)** — qualité pédagogique |
| Configuration Vercel / Supabase / GitHub Actions / DevOps standard | **Modèle rapide économique (cf. lessons.md)** — rapidité |
| Application mécanique de patches sur le doc maître | **Modèle rapide économique (cf. lessons.md)** — travail textuel sans création |
| Tâche jugée non critique en qualité | **Modèle rapide économique (cf. lessons.md)** — économise les quotas Claude raisonnement |

**Règle de pouce (agnostique au modèle, acté v1.8.7)** — Par défaut : le modèle rapide/économique le moins coûteux disponible dans Antigravity. Escalade vers le modèle Claude raisonnement le plus apte UNIQUEMENT si la tâche touche le cœur produit : code React via Stitch, prompts pédagogiques, qualité culturelle marocaine. Le mapping CONCRET et daté (noms de modèles courants) vit dans tasks/lessons.md, jamais hardcodé ici — les noms de modèles sont de l'état volatil.
...
### 14.2.6.5 RETIRÉ (v1.8.8)
Dispositif de proxies de cumul mental supprimé : simulation non fiable. Voir §14.2.9.1 (jauge simplifiée).
...
**Règle technique n°2 — Mention obligatoire du modèle Antigravity.** Tout patch 🎯 POUR ANTIGRAVITY doit commencer par une ligne explicite "MODÈLE RECOMMANDÉ : [modèle courant — cf. tasks/lessons.md, mapping daté]" avec justification courte (1-2 lignes). Conformément à la section 14.2.4. Le mapping concret (noms de modèles courants) est maintenu dans tasks/lessons.md et mis à jour à chaque changement de disponibilité.
...
*Note v1.8.7 : la structure de plans Google AI a évolué depuis (nouveau palier Ultra 100$/mois, top Ultra 250→200$). Le log ci-dessus reste l'historique de la décision du 26/04. Aucun changement de plan acté : on reste sur Pro (§14.4), Overages OFF.*
...
### 14.2.9 Système anti-récidive (acté v1.8.2)

**Justification.** Les leçons documentées dans `tasks/lessons.md` ont un statut **descriptif** (constat de violation passée) mais pas **prescriptif** (règle opposable avant action). Conséquence observée session 08/05/2026 : 6 violations identifiables en une seule session, toutes récidives de leçons documentées. Ce patch transforme les leçons descriptives en règles prescriptives opposables.

#### 14.2.9.1 Jauge de session — simplifiée (v1.8.8, remplace checklist + compteur + proxies)
Honnêteté : Claude ne peut pas mesurer de façon fiable sa saturation de contexte ; compteur mental et comptage d'échanges sont des simulations (constat empirique 26/04, confirmé audit externe 10/06). Seule jauge fiable = le % UI communiqué par le fondateur.
- Après tout livrable lourd : Claude demande le % UI (ex-14.2.6.6, conservé).
- À >= 80% UI déclaré OU décision explicite fondateur OU fin de chantier → synthèse de fin canonique (§14.2.6.7 / RÈGLE 14.2.9.f).
- SUPPRIMÉS : checklist pré-réponse systématique, préfixe compteur d'échanges, GARDE-FOU déclenché par nombre d'échanges auto-compté.

#### 14.2.9.3 Mécanisme 3 — Règles prescriptives (promotion des leçons récidivantes)

Toute leçon ayant fait l'objet de **2 récidives ou plus** est promue en règle prescriptive format SI/ALORS.

**Règles initiales (acté 08/05/2026) :**

- **RÈGLE 14.2.9.a — Routage Antigravity vs Claude Desktop**
  SI livrable > 20 lignes de code OU > 100 lignes markdown OU modification fichier substantielle
  ALORS livrer via patch Antigravity formaté `🎯 POUR ANTIGRAVITY`
  ORIGINE : méta-instructions 8+9 (26/04) + récidive 08/05 — 3 récidives

- **RÈGLE 14.2.9.b — Métrique UI proactive** (FUSIONNÉE v1.8.8 dans §14.2.9.1)
  SI livrable lourd produit (patch, synthèse, fichier > 500 mots)
  ALORS demande explicite métrique UI au fondateur AVANT finalisation réponse
  ORIGINE : 14.2.6.6 + récidives 04/05 + 07/05 + 08/05 — 3 récidives

- **RÈGLE 14.2.9.c — Biais action vs biais menu**
  SI fondateur a tranché une décision claire dans son message
  ALORS livrer immédiatement le livrable, ZÉRO question à choix multiples
  ORIGINE : leçon 26/04 ~21h05 + récidive massive 08/05 — 2 récidives

- **RÈGLE 14.2.9.d — Émission proactive 🔄 GARDE-FOU SESSION** (ASSOUPLIE v1.8.8 : garde-fou émis sur % UI déclaré, plus sur compteur)
  SI 5+ échanges écoulés sans bloc émis
  ALORS émission obligatoire à la prochaine réponse, format strict 4 lignes encadrées
  ORIGINE : leçon 26/04 18h40 + récidive 26/04 ~21h15 + récidive 08/05 — 3 récidives

- **RÈGLE 14.2.9.e — Énumération rituels en ouverture de session** (RETIRÉE v1.8.8 : plus d'énumération rituelle obligatoire en ouverture)
...
- **RÈGLE 14.2.9.h — Anti-sycophantie (promue v1.8.8)**
  SI le fondateur exprime une conviction/décision OU sollicite un avis
  ALORS Claude donne son évaluation honnête y compris en désaccord ; ne valide pas par complaisance, ne flatte ni le système ni les choix du fondateur, et signale ses propres biais (ex. Claude évaluant un livrable produit par Claude).
  ORIGINE : candidate sessions antérieures, preuve empirique 05-10/06 (audit externe + critique méta-gouvernance).

#### 14.2.9.4 Application et révision

- **Application immédiate** dès patch v1.8.2 actif (08/05/2026).
...
### 14.4.6 — Dérogation A1 : première tranche produit fonctionnelle (acté v1.8.7, 2026-06-05)

Le gel de build §14.4 reste ACTIF. Dérogation strictement bornée à la première tranche fonctionnelle (Maths 2Bac SM) :
- AUTORISÉ : runtime LLM en free-tier (ex. Gemini free API) pour un tuteur réellement vivant, SANS achat de crédit ; hébergement Vercel free tier ; backend Supabase free tier.
- INTERDIT maintenu : tout SaaS payant, toute infra cloud lourde/payante, tout achat de crédits IA, et l'activation d'“AI Credit Overages” dans Antigravity.
- SWAP au Go/No-Go : le runtime free-tier est remplacé par Claude Sonnet 4.6 via API Anthropic (runtime prod §3.3) au franchissement du jalon pilote signé + 10 payants. Abstraction provider d'une ligne, prévue dès le build.
- DÉROGATION TEST BORNÉ (acté 2026-06-10) : pour la 1ère tranche (chapitre Limites & Continuité), activation de Claude Sonnet 4.6 (API Anthropic) EN AMONT du Go/No-Go, UNIQUEMENT pour un test borné (<= 5 élèves, ~2 semaines), plafond dur ~600 DH one-off (console Anthropic), quota par élève côté app (réutilisable comme gating freemium). Motif : lever le verrou de fiabilité arithmétique du free-tier (2x3+1=5) non corrigeable par prompt. Dépense bornée non récurrente — discipline zéro-dirham RÉCURRENT intacte.
- Discipline cash inchangée : zéro dirham récurrent engagé avant validation.
...
### 14.6.6 — Recadrage A1 : produit fonctionnel, non démo scriptée (acté v1.8.7, 2026-06-05)

Supersède le cadrage “démo 5 écrans scriptée / données mock”. Le livrable devient la première TRANCHE FONCTIONNELLE de qualité production du produit réel (Maths 2Bac SM), build-for-final intégral.
- Le tuteur (écran 4) est désormais VIVANT (runtime free-tier §14.4.6) : il raisonne, il ne récite pas.
- Le travail déjà déployé (écrans 4, 3, 2 production-grade) est conservé, pas jeté.
- La doctrine anti-distributeur §14.2.9.g devient runtime-opposable : le tuteur live DOIT l'appliquer, plus seulement via une bulle scriptée. À encoder dans le system prompt du runtime.
- Périmètre tranche 1 strictement inchangé : Maths 2Bac SM uniquement. Expansion matières scientifiques (PC, SVT) = backlog post-validation acté.
...
### 14.6.8 — Repivot validation terrain (acté 2026-06-10)
Facteur limitant = absence de contact terrain, pas qualité du cadrage (audit externe + conviction fondateur + entourage).
- Phase 0 interviews LANCÉE MAINTENANT : 15 parents + 15 élèves, zéro prérequis produit, zéro dirham. Donnée manquante depuis 8 versions.
- Produit minimal réel en parallèle : tuteur Sonnet 4.6 + UN chapitre. RAG 1 chapitre = contenu officiel MEN curé en contexte + prompt caching, PAS de pgvector (pgvector = post-Go/No-Go, multi-chapitres).
- École = canal de distribution FUTUR (au chaud), plus canal n°1. One-pager = décrocher un RDV (≠ vente). Conversion réelle attend le produit fonctionnel (marché Maroc : zéro pré-paiement d'un produit non-fonctionnel).
- Cohérence §14.6.7 : aucun nouveau périmètre fonctionnel ; réordonnancement de la séquence de validation. Build freeze inchangé.
