...
> **Version :** 1.0 (création + intégration GitHub MCP)

---

## 0. Vue d'ensemble & objectif

Ce document est le **briefing infra** lu par tout agent IA (Claude, Cursor, Copilot, etc.) au démarrage de session pour opérer sur le projet Bac-IA. Il complète le `Bac IA - Document de Reference Projet.md` (qui couvre le **produit, la pédagogie et la stratégie**), en se concentrant exclusivement sur :
...
1. `Bac IA - Document de Reference Projet.md` → produit, vocabulaire, gouvernance culturelle (Section 2.5)
...
|---|---|
| **Project Name** | `bac-ia` |
| **Project ID** | `prj_bneHlLUGPNmpBUHuOhdiVzcf2Jas` |
| **Build ID (dernier connu)** | `bld_q258t95l7` |
| **Deployment ID (dernier connu)** | `dpl_F9zdMY2s6c4m8zZ8gAqp1KK4YXKU` |
| **Team / Scope Slug** | `nabilbouhria-7135s-projects` |
| **Type de compte** | Hobby personnel (pas de Vercel Team payante) |
| **Framework détecté** | Non détecté — projet statique (HTML/CSS/JS) |

### 1.2 Domaines & URLs

| Environnement | URL |
|---|---|
| **Production (domaine principal)** | `https://bac-ia.vercel.app` |
| **Alias scope** | `https://bac-ia-nabilbouhria-7135s-projects.vercel.app` |
| **Alias branche main** | `https://bac-ia-git-main-nabilbouhria-7135s-projects.vercel.app` |
| **Pattern Preview deployments** | `https://bac-ia-<hash>-nabilbouhria-7135s-projects.vercel.app` |

### 1.3 Infrastructure & Build

| Clé | Valeur |
|---|---|
| **Région de build** | `iad1` — Washington D.C., USA (East) |
| **Machine de build** | 2 cores / 8 GB RAM |
| **Vercel CLI version** | `51.6.1` |
| **Type de déploiement** | `LAMBDAS` |
| **Durée de build** | ~144ms (build) + ~2s (deploy) ≈ 4s total |
| **Cache** | Non configuré (aucun fichier préparé pour le cache) |
...
VERCEL_PROJECT_ID=prj_bneHlLUGPNmpBUHuOhdiVzcf2Jas
...
### 2.4 Appel REST API (exemple)

```bash
curl https://api.vercel.com/v13/deployments/dpl_F9zdMY2s6c4m8zZ8gAqp1KK4YXKU \
...
### 2.6 Fichier `.vercel/project.json` (à créer en local)

```json
{
  "orgId": "nabilbouhria-7135s-projects",
  "projectId": "prj_bneHlLUGPNmpBUHuOhdiVzcf2Jas"
}
```
...
- Connexion native Vercel ↔ GitHub active
- Tout push sur `main` déclenche un déploiement production
...
| Préfixe | Usage |
|---|---|
| `feat(<scope>):` | Nouvelle fonctionnalité (ex: `feat(lot2):`, `feat(diagnostic):`) |
...
- Référencer le scope projet quand pertinent : `feat(lot2)`, `fix(onboarding)`, `docs(infra)`
...
1. **Aucun push direct sur `main`** pour des modifications de **code applicatif** (HTML, JS, CSS, JSON de config produit). Passer obligatoirement par PR.
...
Les modifications suivantes peuvent être pushées directement sur `main` après validation explicite dans le chat :
- Documentation (`*.md` à la racine, hors `LICENSE`/`SECURITY.md`)
- Fichiers de contexte agent (`INFRA_CONTEXT.md`, futurs `AGENT_*.md`)
- Mises à jour du `Bac IA - Document de Reference Projet.md` (sous gouvernance Section 2.5)
...
## Test
[Comment vérifier que ça marche — URL preview Vercel, étapes manuelles, etc.]
...
- 🌍 Région de build `iad1` (US East) — **envisager `cdg1` (Paris)** pour réduire la latence Maroc (~40-50% gain estimé)
- 📦 Cache de build non configuré — l'ajout d'un framework (Next.js, Vite) activerait le cache auto
